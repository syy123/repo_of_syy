...
class ChannelSource(wx.Panel):
    ...
    def AddElementForLeftSizer(self):        
        self.left_vsizer.Add(self.symbolNumberBox, 0, wx.ALL|wx.EXPAND, 1)
        self.left_vsizer.Add(self.symbolRateBox, 0, wx.ALL|wx.EXPAND, 1)       
        self.left_vsizer.Add(self.sampleBox, 0, wx.ALL|wx.EXPAND, 1)     
        self.left_vsizer.Add(self.ImpulseChoiceSizer, 0, wx.ALL|wx.EXPAND, 1)
        self.left_vsizer.Add(self.RaisedCosineSizer, 0, wx.ALL|wx.EXPAND, 1)
        self.left_vsizer.Add(self.partialBox, 0, wx.ALL|wx.EXPAND, 2)
        self.left_vsizer.Add(self.gaussBox, 0, wx.ALL|wx.EXPAND, 2)
        self.left_vsizer.Add(self.CodeChoiceSizer, 0, wx.ALL|wx.EXPAND, 1) 
        self.left_vsizer.Add(self.rectRatioBox, 0, wx.ALL|wx.EXPAND, 1)
    ...
    def ConfigSizersLayout(self):
        self.TopSizer.Add(self.left_vsizer, 1.5, wx.ALL|wx.EXPAND, 1)
    ...
    def SymbolRateBox(self):
        self.staticSymbolRateBox = wx.StaticBox(self, -1, u"码元速率")
    ...
    def ChoiceBoxSwitchFunc(self, event):
        if self.impulseChoiceBox.GetLabelText() == RaisedCosineBoxName():
            print u'升余弦脉冲'
            self.alphBox.slider.Enable()
            self.typeChoice.Disable()
            self.sigmaBox.slider.Disable()
            self.codeChoiceBox.Disable()
            self.ratioBox.slider.Disable()
        elif self.impulseChoiceBox.GetLabelText() == PartialBoxName():
            print u'部分响应波形'
            self.typeChoice.Enable()
            self.alphBox.slider.Disable()
            self.sigmaBox.slider.Disable()
            self.codeChoiceBox.Disable()
            self.ratioBox.slider.Disable()
        elif self.impulseChoiceBox.GetLabelText() == GaussBoxName():
            print u'高斯脉冲'
            self.sigmaBox.slider.Enable()
            self.typeChoice.Disable()
            self.alphBox.slider.Disable()
            self.codeChoiceBox.Disable()
            self.ratioBox.slider.Disable()
            ...
            print u'矩形脉冲'
            self.codeChoiceBox.Enable()
            self.sigmaBox.slider.Disable()
            self.typeChoice.Disable()
            self.alphBox.slider.Disable()
            self.ratioBox.slider.Enable()
